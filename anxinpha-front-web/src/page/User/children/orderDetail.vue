      ...
      <div slot="content">
        <div v-loading="loading" element-loading-text="加载中..." style="min-height: 10vw;" v-if="orderList.length">
          ...
          <div class="status-now" v-if="orderStatus === 2">
            <ul>
              <li class="status-title"><h3>订单状态：已支付，等待商家发货</h3></li>
            </ul>
            <p class="realtime">
              <span>请耐心等待，商家正在火速发货中，，，，</span>
            </p>
          </div>
          ...
          <div class="order-discount-line">
            ...
            <y-button v-if="orderStatus === 4" text="确认收货" @btnClick="_confirmReceipt"
...
<script>
  import { getOrderDet, cancelOrder, confirmReceipt, payMent } from '/api/goods'
      ...
      _getOrderDet () {
        ...
        getOrderDet(params).then(res => {
          if (res.orderStatus === '0') {
            this.orderStatus = 1
          } else if (res.orderStatus === '1' || res.orderStatus === '2') {
            this.orderStatus = 2
          ...
          this.userName = res.addressInfo.userName
          this.tel = res.addressInfo.tel
          this.streetName = res.addressInfo.streetName
